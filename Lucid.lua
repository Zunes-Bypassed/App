...
Components.Notification = (function()
    ...
    function Notification:New(Config)
        ...
        NewNotification.CloseButton.MouseEnter:Connect(function()
            game:GetService("TweenService"):Create(CloseIcon, TweenInfo.new(0.2), { ImageColor3 = Color3.fromRGB(255, 120, 120) }):Play()
        end)
        NewNotification.CloseButton.MouseLeave:Connect(function()
            game:GetService("TweenService"):Create(CloseIcon, TweenInfo.new(0.2), { ImageColor3 = Color3.fromRGB(220, 220, 220) }):Play()
        end)
...
function Element:New(Idx, Config)
    ...
    local function RecalculateListPositionAndSize()
        ...
        local frameBottom = Library.GUI and (Library.GUI.AbsolutePosition.Y + Library.GUI.AbsoluteSize.Y) or viewportY
        ...
        if finalY + finalHeight > frameBottom then finalHeight = frameBottom - finalY end
        local finalX = math.clamp(absPos.X, 0, viewportX - DropdownHolderFrame.AbsoluteSize.X)
    ...
    function Dropdown:GetActiveValues()
        if self.Multi then
            local count = 0
            for _, v in pairs(self.Value) do if v then count = count + 1 end end
            return count
        else
    ...
    function Dropdown:Open() Dropdown.Opened = true ScrollFrame.ScrollingEnabled = false DropdownHolderCanvas.Visible = true task.defer(RecalculateListPositionAndSize) end
    function Dropdown:Close() Dropdown.Opened = false ScrollFrame.ScrollingEnabled = true DropdownHolderCanvas.Visible = false end
    ...
    function Dropdown:BuildDropdownList()
        local Values = Dropdown.Values
        for _, Element in next, DropdownScrollFrame:GetChildren() do if not Element:IsA("UIListLayout") then Element:Destroy() end end
        local Buttons = {}
